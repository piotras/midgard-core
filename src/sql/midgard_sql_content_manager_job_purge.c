...
static void
_midgard_sql_content_manager_job_purge_executable_iface_execute (MidgardExecutable *iface, GError **error)
{
	GError *err = NULL;
	...
	/* purge object */
	midgard_object_purge(content_object, FALSE, &err);
	if (err) {
		g_propagate_error (error, err);
		failed = TRUE;
	}
...
static void
_midgard_sql_content_manager_job_purge_executable_iface_execute_async (MidgardExecutable *iface, GError **error)
{
	GError *err = NULL;
	...
	/* purge object */
	gboolean rv =  _midgard_object_purge (content_object, FALSE, &err);
	if (err)
		g_propagate_error (error, err);
...
static GObject *
_midgard_sql_content_manager_job_purge_constructor (GType type,
...
static void
_midgard_sql_content_manager_job_purge_dispose (GObject *object)
{
	parent_class->dispose (object);
}

static void 
_midgard_sql_content_manager_job_purge_finalize (GObject *object)
{
	parent_class->finalize (object);
}

static void 
_midgard_sql_content_manager_job_purge_class_init (MidgardSqlContentManagerJobPurgeClass *klass, gpointer class_data)
{
       	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	parent_class = g_type_class_peek_parent (klass);

	object_class->constructor = _midgard_sql_content_manager_job_purge_constructor;
	object_class->dispose = _midgard_sql_content_manager_job_purge_dispose;
	object_class->finalize = _midgard_sql_content_manager_job_purge_finalize;
}

/* Executable iface */
static void
_midgard_sql_content_manager_job_purge_executable_iface_init (MidgardExecutableIFace *iface)
{
	iface->execute = _midgard_sql_content_manager_job_purge_executable_iface_execute;
	iface->execute_async = _midgard_sql_content_manager_job_purge_executable_iface_execute_async;
}

/* Job iface */
static void
_midgard_sql_content_manager_job_purge_job_iface_init (MidgardJobIFace *iface)
{
...
static void
_midgard_sql_content_manager_job_purge_content_manager_job_iface_init (MidgardContentManagerJobIFace *iface)
{
...
/* Validable iface */
static void
_midgard_sql_content_manager_job_purge_validable_iface_init (MidgardValidableIFace *iface)
{
...
GType
midgard_sql_content_manager_job_purge_get_type (void)
{
   	static GType type = 0;
	if (type == 0) {
		static const GTypeInfo info = {
			sizeof (MidgardSqlContentManagerJobPurgeClass),
			NULL,           /* base_init */
			NULL,           /* base_finalize */
			(GClassInitFunc) _midgard_sql_content_manager_job_purge_class_init,
			NULL,           /* class_finalize */
			NULL,           /* class_data */
			sizeof (MidgardSqlContentManagerJobPurge),
			0,              /* n_preallocs */
			NULL		/* instance_init */
		};

		static const GInterfaceInfo executable_info = {
			(GInterfaceInitFunc) _midgard_sql_content_manager_job_purge_executable_iface_init,
			NULL,   /* interface_finalize */
			NULL    /* interface_data */
		};

		static const GInterfaceInfo job_info = {
			(GInterfaceInitFunc) _midgard_sql_content_manager_job_purge_job_iface_init,
			NULL,   /* interface_finalize */
			NULL    /* interface_data */
		};

		static const GInterfaceInfo content_manager_job_info = {
			(GInterfaceInitFunc) _midgard_sql_content_manager_job_purge_content_manager_job_iface_init,
			NULL,   /* interface_finalize */
			NULL    /* interface_data */
		};

		static const GInterfaceInfo validable_info = {
			(GInterfaceInitFunc) _midgard_sql_content_manager_job_purge_validable_iface_init,
			NULL,   /* interface_finalize */
			NULL    /* interface_data */
		};

		type = g_type_register_static (MIDGARD_TYPE_SQL_CONTENT_MANAGER_JOB, "MidgardSqlContentManagerJobPurge", &info, 0);
